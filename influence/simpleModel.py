...
from influence.simple_kerasinstance import SimpleCNN
...
class BinarySimpleModel(GenericNeuralNet):

    def __init__(self, img_side, num_channels, weight_decay, weight_path, num_features **kwargs):
        self.weight_decay = weight_decay

        self.img_side = img_side
        self.num_channels = num_channels
        self.input_dim = img_side * img_side * num_channels
        self.num_features = num_features # Hardcoded for inception. For some reason Flatten() doesn't register num_features.

        super(BinaryInceptionModel, self).__init__(**kwargs)

        self.load_saved_weights(weight_path)
    ...
    def load_saved_weights(self, weight_path):
        ...
        self.simple_model.load_weights(weight_path)


    def inference(self, input):
        reshaped_input = tf.reshape(input, [-1, self.img_side, self.img_side, self.num_channels])
        self.simple_model = SimpleCNN(include_top=False, weights=None, input_tensor=reshaped_input)

        raw_inception_features = self.simple_model.output
